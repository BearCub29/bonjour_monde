...
=begin
#--------first_choice5_tweet5-----------#
choices = arr.sample(5)
...
choices.each do |t|
  client.update("Bonjour #{t} ! Merci pour ces moments de partage culturel.#bonjour_monde")
end
...
#--------second_like_bonjourmonde-----------#



client.search("#bonjour_monde -rt", result_type: "recent").take(25).each do |tweet|
...
def follow_lasts(client)
  hand=[]
  client.search('#bonjour_monde', result_type: "recent").take(200).each do |tweet|
    hand << tweet.user.screen_name
  end

  hand = hand.uniq.delete_if{|n| n == "FloPlch"}
  ...
  (0..20).each do |i|
    ...
    if i < hand.length
      ...
      client.follow(hand[i])
      puts "followed : " + hand[i]
